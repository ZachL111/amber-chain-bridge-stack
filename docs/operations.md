...
`amber-chain-bridge-stack` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for bridge graph analysis, using node-edge fixtures and cycle and reachability reports.
...
The score is deterministic. The current threshold is 158. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 5.
